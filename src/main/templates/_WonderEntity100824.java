...
#foreach ($relationship in $entity.sortedClassToManyRelationships)
  ...
  public void set${relationship.capitalizedName}(NSArray<$relationship.actualDestination.classNameWithDefault> aValue) {
	  	if (${entity.prefixClassNameWithoutPackage}.LOG.isDebugEnabled()) {
	      ${entity.prefixClassNameWithoutPackage}.LOG.debug("updating ${relationship.name} from "+ ${relationship.name}()+" to "+aValue);
	    }
	        takeStoredValueForKey(aValue, ${entity.prefixClassNameWithoutPackage}.${relationship.uppercaseUnderscoreName}_KEY);
	  }
  ...
  public void deleteAll${relationship.capitalizedName}Relationships() {
	...
	public static $entity.classNameWithOptionalPackage createEO${entity.classNameWithoutPackage}( EOEditingContext context )
	{
...
#foreach ($binding in $fetchSpecification.distinctBindings)
	...
	//Return the list of changes between the current EO and the last committed version of the EO (Chuck Hill)
